 ...
 * @param {function(new: _contextTesting.Context)|Array<function(new: _contextTesting.Context)>|*} [conf.context] The single or multiple context constructors or objects to initialise for each test.
 * @param {function(new: _contextTesting.Context)|Array<function(new: _contextTesting.Context)>|*} [conf.persistentContext] The context constructor(s) that will be initialised and destroyed once per test suite, having a persistent state across tests.
 * @param {function(this:_contextTesting.MaskContext, ..._contextTesting.Context): *|!Promise} [conf.getResults] A possibly async function which returns results of a test. If it outputs a string, it will be compared against the `expected` property of the mask using string comparison. If it outputs an object, its deep equality with `expected` can be tested by adding `'expected'` to the `jsonProps`. Otherwise, the result must be mapped for comparison with `expected` using the `mapActual` method.
 * @param {function(this:_contextTesting.MaskContext, ..._contextTesting.Context): !(stream.Transform|Promise<!stream.Transform>)} [conf.getTransform] A possibly async function which returns a _Transform_ stream to be ended with the input specified in the mask's result. Its output will be accumulated and compared against the expected output of the mask.
 * @param {function(this:_contextTesting.MaskContext, ..._contextTesting.Context): !(stream.Readable|Promise<stream.Readable>)} [conf.getReadable] A possibly async function which returns a _Readable_ stream constructed with the input from the mask. Its output will be stored in memory and compared against the expected output of the mask.
 ...
 * @param {function (this:_contextTesting.MaskContext, ..._contextTesting.Context): _assertThrows.Config} [conf.getThrowsConfig] A function which should return a configuration for [`assert-throws`](https://github.com/artdecocode/assert-throws), including `fn` and `args`, when testing an error.
